...
) AS v(title, description, nodes, edges, is_template, user_id)
WHERE NOT EXISTS (
  SELECT 1 FROM public.workflows WHERE title = v.title AND is_template = true
);

-- Drop and recreate RLS policy for viewing workflows
DROP POLICY IF EXISTS "Users can view their own workflows" ON public.workflows;
DROP POLICY IF EXISTS "Users can view templates and their own workflows" ON public.workflows;

CREATE POLICY "Users can view templates and own workflows"
ON public.workflows
FOR SELECT
